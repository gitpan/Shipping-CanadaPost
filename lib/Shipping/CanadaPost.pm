...
Shipping::CanadaPost - A module to fetch shipping costs for Canada Post.

=head1 SYNOPSIS

	use Shipping::CanadaPost;
	
	#initialise object - specifying from postal code, and canada post merchant id
	my $shiprequest = Shipping::CanadaPost->new(	merchantid => 'CPC_DEMO_XML',
							frompostal => 'M1P1C0',
							testing	   => 1			);
...
Shipping::CanadaPost is a Perl library created to allow users to fetch real-time options and pricing quotes
...
Creates a new Shipping::CanadaPost object.  Different objects available are:
